...
function PswdInput(props) {
  ...
  const staticRevealList = useCallback(
    () => ({
      position: "absolute",
      right: "right-2",
      top: "top-1/2",
  ...
  return (
    <>
      <div
        ...
        <span
          className={`
          ${Object.values(staticRevealList).join(" ")}
          ${Object.values(dynamicSpanList).join(" ")}`}
          onClick={() => setReveal(!reveal)}>
          {reveal ? (
            <i className="far fa-eye-slash text-sm cursor-pointer"></i>
          ) : (
            <i className="far fa-eye text-sm cursor-pointer"></i>
          )}
        </span>
      </div>
      {/* {error && (
